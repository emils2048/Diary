...
<%@ include file="/sidemenu.jsp" %>
    ...
    <div class="write-feed-card">
        <div class="write-feed-header">
            <img src="/img/${view.member.profile }" class="write-profile-img" alt="프로필">
            <div class="write-profile-info">
                <div class="write-nickname">${view.member.username }</div>
                <div class="write-date">${view.regdate }</div>
            </div>
        </div>
        <form method="post" action="${pageContext.request.contextPath}/bc/diaryupdatepro.do"
            ...
            <div class="form-group w-100" style="margin-top: 18px;">
                <textarea class="form-control write-content-input" id="content" name="content" 
                          ...
                          style="font-size: 1.18em; height: 320px;">${view.content }</textarea>
            </div>
            <div class="write-feed-actions">
                ...
                <button type="submit" class="write-btn" style="background:#987de8; color:#fff; border:none; border-radius:16px; padding:7px 22px; font-size:1em; font-weight:600; cursor:pointer;">投稿する</button>
            </div>
        </form>
    </div>
</div>
